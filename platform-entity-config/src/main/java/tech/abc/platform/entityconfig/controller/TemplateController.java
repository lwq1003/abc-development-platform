...
public class TemplateController extends BaseController {
    ...
    @SystemLog(value = "模板-自定义查询")
    @PreAuthorize("hasPermission(null,'entityconfig:template:query')")
    public ResponseEntity<Result> customQuery(@RequestBody String customQueryString, PageInfo pageInfo, SortInfo sortInfo) {
        // 构造分页对象
        IPage<Template> page = new Page<Template>(pageInfo.getPageNum(), pageInfo.getPageSize());
        // 构造查询条件
        QueryWrapper<Template> queryWrapper = CustomQueryGenerator.generateQueryWrapper(Template.class, customQueryString, sortInfo);
    ...
    protected List<TemplateVO> convert2VO(List<Template> entityList) {
        ...
        // 获取 组织机构 集合
        List<String> organizaitonList = entityList.stream().map(x -> x.getOrganizaiton()).collect(Collectors.toList());
        Map<String, String> organizaitonNameMap = organizationService.getNameMap(organizaitonList);

        entityList.stream().forEach(x -> {
            TemplateVO vo = convert2VO(x);
            // 设置 组织机构
            vo.setOrganizaitonName(organizaitonNameMap.get(x.getOrganizaiton()));
            voList.add(vo);
        });
        return voList;
    }
