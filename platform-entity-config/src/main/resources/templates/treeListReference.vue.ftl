<template>
    <div class="w-full">
        ...
        <Dialog title="${table.comment!}——选择" v-model="visible" width="80%">
                ...
                <el-main>
                    <CollapseTab>
                        <el-form :inline="true" :model="queryCondition" label-width="80px" @keyup.enter="query">
                            <!--查询条件区 -->
                            <#list queryConditionListTreeListReference as item>
                            <#--根据数据类型处理-->
...
<script>
    ...
    <#if treeListReferenceEntityView.treePath??>
    import TreeView from '@/modules/${treeListReferenceEntityView.treePath}'
    <#else>
    ...
    export default {
        ...
        data() {
            return {
                ...
                columnList: [
                    <#list queryResultListTreeListReference as field>
                    {
        ...
        methods: {
            <#if treeListReferenceEntityView.beforeInit?? && treeListReferenceEntityView.beforeInit!="">
            beforeInit(param) {
                ${treeListReferenceEntityView.beforeInit}
            },
            </#if>
            <#if treeListReferenceEntityView.afterInit?? && treeListReferenceEntityView.afterInit!="">
            afterInit(param) {
                ${treeListReferenceEntityView.afterInit}
            },
            </#if>
            <#if treeListReferenceEntityView.commonParamChange?? && treeListReferenceEntityView.commonParamChange!="">
            commonParamChange(param) {
                ${treeListReferenceEntityView.commonParamChange}
            },
            </#if>
        }
    }
