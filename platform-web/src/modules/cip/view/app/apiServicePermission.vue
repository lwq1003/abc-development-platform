<template>
  <Dialog title="API服务权限配置" v-model="visible" width="1000">
    <CollapseTab>
      <el-form :inline="true" :model="queryCondition" label-width="120px" @keyup.enter="query">
        ...
        <el-form-item label="分类">
          <dictionary-select v-model="queryCondition.category" code="ApiServiceCategory" multiple />
        </el-form-item>
    ...
    <div class="mb-10px mt-10px">
      <el-button
        v-permission="'cip:app:grantApiPermission'"
        type="primary"
        @click="grantApiPermission"
        >授权</el-button
      >
      <el-button
        v-permission="'cip:app:withdrawApiPermission'"
        type="primary"
  ...
  methods: {
    ...
    grantApiPermission() {
      ...
      this.$confirm('是否授权?', '确认', {
        ...
        .then(() => {
          ...
          this.$api.cip.app.grantApiPermission(this.appId, ids).then(() => {
            this.refresh()
          })
        })
        .catch(() => {
    ...
    withdrawApiPermission() {
      ...
      this.$confirm('是否授权?', '确认', {
        ...
        .then(() => {
          const ids = this.getCheckedIdList()
          this.$api.cip.app.withdrawApiPermission(this.appId, ids).then(() => {
            this.refresh()
          })
        })
        .catch(() => {
