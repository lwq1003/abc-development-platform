...
export default {
  ...
  methods: {
    ...
    // 移除
    remove(propertyId) {
      this.$confirm('此操作将移除视图属性, 是否继续?', '确认', {
        type: 'warning'
      })
        .then(() => {
          this.$api.entityconfig.viewProperty.remove(propertyId).then(() => {
            this.query()
          })
        })
        .catch(() => {
    ...
    // 加载列表
    query() {
      this.$api.entityconfig.viewProperty.listByView(this.entityViewId).then((res) => {
        this.propertityList = res.data
      })
    },
    // 拖拽结束
    updateSort(evt) {
    ...
    // 排序
    sort() {
      const sortedList = this.propertityList.map(function (value, index) {
        return { index: index, code: value.code }
      })
      this.$api.entityconfig.viewProperty.updateSort(this.entityViewId, sortedList).then(() => {
        this.query()
      })
    },
    // 清空
    clear() {
      this.$confirm('此操作将移除所有属性,已配置信息丢失且不可恢复，是否继续?', '确认', {
        type: 'warning'
      })
        .then(() => {
          this.$api.entityconfig.viewProperty.clear(this.entityViewId).then(() => {
            this.query()
          })
        })
        .catch(() => {
