        ...
        <template #header>
          <div class="card-header">
            ...
            <span style="float: right">
              <el-button style="padding: 0; border: 0" icon="right" @click="addQueryCondition" />
              <el-button
                style="padding: 0; border: 0"
                icon="bottom-right"
                @click="addQueryResult"
              />
            </span>
          </div>
        </template>
...
export default {
  ...
  methods: {
    ...
    addQueryCondition() {
      this.$confirm(
        '此操作将把所有属性批量添加作为查询条件,已添加和配置的查询条件不会覆盖，是否继续?',
        '确认',
        {
          type: 'warning'
        }
        ...
        .then(() => {
          this.$api.entityconfig.viewQueryCondition
            .addBatchFromModelProperty(this.entityViewId)
            .then(() => {
              this.$emit('refresh-query-condition')
            })
        })
        .catch(() => {
    ...
    addQueryResult() {
      this.$confirm(
        '此操作将把所有属性批量添加作为查询结果,已添加和配置的查询结果不会覆盖，是否继续?',
        '确认',
        {
          type: 'warning'
        }
        ...
        .then(() => {
          this.$api.entityconfig.viewQueryResult
            .addBatchFromModelProperty(this.entityViewId)
            .then(() => {
              this.$emit('refresh-query-result')
            })
        })
        .catch(() => {
          this.$message.info('已取消')
