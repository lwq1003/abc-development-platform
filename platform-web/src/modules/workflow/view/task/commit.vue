      ...
      <el-row v-if="showCommonComment">
        <el-col :span="24">
          <el-form-item label="审批意见" prop="comment">
            <el-input v-model="taskData.comment" type="textarea" :rows="3" style="width: 220px" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row v-show="nextStepVisible">
        <el-col :span="24">
...
export default {
  ...
  methods: {
    show(processInstanceId, taskId, delegation) {
      this.taskData.taskId = taskId
      this.taskData.processInstanceId = processInstanceId
      this.getForwardNodeList()
      // 如为被委派，则不显示下一环节
      if (delegation && delegation === 'PENDING') {
    ...
    commit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // 执行流程提交操作
          this.$api.workflow.task.commit(this.taskData).then(() => {
            this.$emit('success')
            this.close()
          })
        }
    ...
    getForwardNodeList() {
      this.$api.workflow.task.getForwardNodeList(this.taskData.taskId).then((res) => {
